      ...
      >
        <div className="mb-8 border-b border-white/[0.04] pb-6">
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">
            Apply for <span className="bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent">{companyName}</span>
          </h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 ">
            ...
            <Fieldset.Group className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="flex flex-col gap-1.5">
                <Input 
                  ...
                  className="w-full"
                />
                {errors.fullName && <p className="text-[11px] text-rose-500 font-medium pl-1">{errors.fullName.message}</p>}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                <Input 
                  ...
                  variant="bordered"
                />
                {errors.email && <p className="text-[11px] text-rose-500 font-medium pl-1">{errors.email.message}</p>}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                <Input 
                  ...
                  variant="bordered"
                />
                {errors.phone && <p className="text-[11px] text-rose-500 font-medium pl-1">{errors.phone.message}</p>}
              </div>
            ...
            <Fieldset.Group className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    >
                      ...
                      <Select.Trigger className="w-full bg-transparent border border-[#262626] rounded-xl text-sm min-h-10 text-left px-3 flex items-center justify-between hover:border-white/10 transition-colors">
                        <Select.Value className="text-gray-400" placeholder="Select experience range" />
                        <Select.Indicator />
                      </Select.Trigger>
                      <Description className="text-[10px] text-gray-500 mt-1 block">Select your total track years</Description>
                      
                      <Select.Popover className="bg-[#121212] border border-[#262626] text-white rounded-xl shadow-2xl mt-1">
                        <ListBox className="p-1">
                          <ListBox.Item key="entry" className="hover:bg-white/5 rounded-lg p-2 text-xs transition-colors cursor-pointer">
                            <Label className="font-semibold block cursor-pointer">Entry Level</Label>
                            <Description className="text-[10px] text-gray-400 block">0 - 1 Year of experience</Description>
                          </ListBox.Item>
                          <ListBox.Item key="mid" className="hover:bg-white/5 rounded-lg p-2 text-xs transition-colors cursor-pointer">
                            <Label className="font-semibold block cursor-pointer">Mid Level</Label>
                            <Description className="text-[10px] text-gray-400 block">2 - 5 Years of experience</Description>
                          </ListBox.Item>
                          <ListBox.Item key="senior" className="hover:bg-white/5 rounded-lg p-2 text-xs transition-colors cursor-pointer">
                            <Label className="font-semibold block cursor-pointer">Senior Level</Label>
                            <Description className="text-[10px] text-gray-400 block">5+ Years of experience</Description>
                          </ListBox.Item>
                        </ListBox>
                      </Select.Popover>
                    </Select>
                    {errors.experience && <p className="text-[11px] text-rose-500 font-medium pl-1">{errors.experience.message}</p>}
                  </div>
                )}
              />
              ...
              <div className="md:col-span-2">
                <Input 
                  ...
                  variant="bordered"
                />
                {errors.skills && <p className="text-[11px] text-rose-500 font-medium pl-1">{errors.skills.message}</p>}
              </div>

              <Input {...register("portfolio")} label="Portfolio Website (Optional)" placeholder="https://myportfolio.com" variant="bordered" />
              <Input {...register("linkedin")} label="LinkedIn Profile (Optional)" placeholder="https://linkedin.com/in/username" variant="bordered" />
              <div className="md:col-span-2">
                <Input {...register("github")} label="GitHub Profile" placeholder="https://github.com/username" variant="bordered" />
              </div>
            </Fieldset.Group>
          </Fieldset>
          ...
          <Fieldset className="border-none p-0 m-0 space-y-4">
            ...
            <Fieldset.Group className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="flex flex-col gap-2">
                <span className="text-xs text-gray-400 font-medium">Resume/CV (Required)</span>
                <label className="border border-dashed border-[#262626] hover:border-white/20 transition-colors rounded-xl p-4 flex items-center justify-center gap-3 cursor-pointer bg-white/[0.01]">
                  ...
                  <span className="text-xs font-semibold text-gray-300">Upload PDF, DOCX (Max 5MB)</span>
                  <input type="file" required className="hidden" accept=".pdf,.docx" />
                </label>
              </div>

              <div className="flex flex-col gap-2">
                <span className="text-xs text-gray-400 font-medium">Cover Letter (Optional)</span>
                <label className="border border-dashed border-[#262626] hover:border-white/20 transition-colors rounded-xl p-4 flex items-center justify-center gap-3 cursor-pointer bg-white/[0.01]">
                  <UploadCloud size={18} className="text-gray-500" />
                  <span className="text-xs font-medium text-gray-400">Upload Document / Pitch</span>
          ...
          <Fieldset className="border-none p-0 m-0 space-y-4">
            ...
            <Fieldset.Group className="space-y-5">
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-xs text-gray-400 font-medium mb-1 block">Why are you interested in this position?</span>
                <TextArea
                  {...register("interestStatement", { required: "Please express your motivation" })}
                ...
                />
                {errors.interestStatement && <p className="text-[11px] text-rose-500 font-medium pl-1">{errors.interestStatement.message}</p>}
              </div>
